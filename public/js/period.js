...
async function loadDB(){
  try{
    ...
    console.log("[period.js] DB loaded ✅", {
      todayMain: fortuneDB?.today?.main?.length || 0,
      tomorrowMain: fortuneDB?.tomorrow?.main?.length || 0,
...
function getSectionDB(type){
  if(type === "today") return fortuneDB?.today || {};
  if(type === "tomorrow") return fortuneDB?.tomorrow || {};
  return fortuneDB?.year || {};
}
...
function buildDeepFortune(type){
  ...
  let targetKey = "";
  if(type === "today"){
    targetKey = formatDateLocal(getTodayTargetDate());
  }else if(type === "tomorrow"){
    targetKey = formatDateLocal(getTomorrowTargetDate());
  }else{
    targetKey = String(ACTIVE_YEAR);
  }

  const seedBase = buildSeed(birth, targetKey, type);
  const section = getSectionDB(type);
  ...
  return {
    ...
    luckyNumber: seededPick(section?.lucky_number, seedBase + "|luckyNumber", String(seededNumber(seedBase + "|number", 1, 33))),
...
function showToday(_fromClick = false){
  ...
  const content = seededPick(
    fortuneDB?.today?.main,
    buildSeed(birth, targetKey, "today-main"),
...
function showTomorrow(_fromClick = false){
  ...
  const content = seededPick(
    fortuneDB?.tomorrow?.main,
    buildSeed(birth, targetKey, "tomorrow-main"),
